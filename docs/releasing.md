...
3. Run `npm ci` and `npm run check` (tests, type checking, lint, architecture, solver work counters, formatting and example regeneration; `git diff examples/` must then be empty), then `npm run coverage`, `npm run build`, `npm run build:vercel`, `npm run validate:vercel` and `python scripts/validate_scipy.py --check`.
