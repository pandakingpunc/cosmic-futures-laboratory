...
3. Run `npm ci` and `npm run check` (tests, type checking, lint, architecture, solver work counters, formatting and example regeneration; `git diff examples/` must then be empty), then `npm run coverage`, `npm run build`, `npm run build:vercel`, `npm run validate:vercel`, `python scripts/validate_scipy.py --check` and `python scripts/pow_reference.py --check`.
4. Review the author metadata, update the version and actual release date in the package and citation files and the `CHANGELOG.md` heading, and remove the previous version's top-level `doi` from `CITATION.cff` until Zenodo assigns the new version DOI. Run `npm run release:check`. It fails when a version, dataset version, release date, title or license disagrees, when the top-level `doi` belongs to another version or is not listed under `identifiers`, or when placeholders remain; it does not create a GitHub repository or publish a Zenodo record.
